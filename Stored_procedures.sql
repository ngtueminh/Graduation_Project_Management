...
--6 
-- Calculate the average score for each project, using the CURSOR
GO
CREATE PROCEDURE TINHDTB @DTB FLOAT OUTPUT
AS
BEGIN
	DECLARE @DUYET CURSOR, @MSDT CHAR(6),@AVG FLOAT
	...
	WHILE @@FETCH_STATUS =0
		...
		WHERE MSDT = @MSDT
		INSERT INTO DETAI_DIEM VALUES (@MSDT,@AVG)
		FETCH NEXT FROM @DUYET INTO @MSDT
		END
	CLOSE @DUYET
	DEALLOCATE @DUYET
	SELECT @DTB=AVG(DIEM) FROM DETAI_DIEM
END
GO
DECLARE @DTB FLOAT
EXEC TINHDTB @DTB OUTPUT
CREATE VIEW BTAP1B
AS
SELECT DT.MSDT, TENDT, UV.MSGV, TENGV, DIEM
FROM DETAI DT, GV_UVDT UV, GIAOVIEN GV
WHERE DT.MSDT = UV.MSDT AND UV.MSGV = GV.MSGV
ORDER BY DT.MSDT, DIEM DESC OFFSET 0 ROWS